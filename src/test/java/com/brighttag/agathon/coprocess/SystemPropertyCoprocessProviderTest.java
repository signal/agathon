...
public class SystemPropertyCoprocessProviderTest extends EasyMockSupport {
  ...
  private CassandraInstanceService instanceService;
  ...
  @Before
  public void setUp() {
    instanceService = createMock(CassandraInstanceService.class);
    service = new SystemPropertyCoprocessProvider(instanceService);
  }
  ...
  public void getCassandraCoprocess() {
    ...
    expect(instanceService.findById(CASSANDRA_ID)).andReturn(coprocess);
    replayAll();
  ...
  public void getCassandraCoprocess_instanceNotInDb() {
    expect(instanceService.findById(CASSANDRA_ID)).andReturn(null);
    replayAll();
